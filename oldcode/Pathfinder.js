class Pathfinder extends Phaser.Scene {
        ...
        for (let y = 0; y < this.map.height; y++) {
            ...
            for (let x = 0; x < this.map.width; x++) {
                this.map.layers.forEach(layer => {
                    ...
                    if (layer.tilemapLayer.getTileAt(x, y) != null) {
                        grid[y][x] = layer.tilemapLayer.getTileAt(x, y).index;
                    }
                });
            }
        }
    ...
    setCost(tileset) {
        for (let i = tileset.firstgid; i <= tileset.total; i++) {
            let properties = tileset.getTileProperties(i);
            if (properties == null) continue;
            this.finder.setTileCost(i, properties.cost);
        }
    }
}
